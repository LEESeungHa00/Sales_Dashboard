...
if 'date_range' in locals() and df is not None and not df.empty:
    ...
    start_date = korea_tz.localize(start_datetime)
    end_date = korea_tz.localize(end_datetime)
    ...
    with tab1:
        ...
        with col1:
            ...
            if len(funnel_data) > 1:
                ...
                if missing_cols:
                    st.warning(f"**아래 컬럼이 없어 Funnel을 그릴 수 없습니다:**\n\n - " + "\n - ".join(missing_cols))
        ...
        with col3:
            st.markdown("**Account Size 분포**")
            if 'account_size' in base_df.columns and not base_df.empty:
                account_size_counts = base_df['account_size'].value_counts().reset_index()
                account_size_counts.columns = ['Account Size', 'Count']
                
                fig_pie = px.pie(
                    account_size_counts, 
                    values='Count', 
                    names='Account Size', 
                    title='전체 딜 Account Size 분포',
                    hole=0.4
                )
                fig_pie.update_traces(textposition='inside', textinfo='percent+label')
                st.plotly_chart(fig_pie, use_container_width=True)
            else:
                st.info("Account Size 데이터를 불러오지 못했습니다. HubSpot 연동 설정을 확인하세요.")
